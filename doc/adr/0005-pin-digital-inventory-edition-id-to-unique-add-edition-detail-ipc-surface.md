...
- `<wa-tab-group>` is introduced in two places (peek + route). Any
  ...
  in both.
- The `PrimaryKey` enum in `core/livet-data` now mixes composite PKs
  (junction tables) with single-column UNIQUE indexes (m0011). The
  enum's doc comment was updated to reflect this. A latent footgun:
  the `strum(prefix = "pk_")`-derived `Display` produces
  `pk_digital_inventory_edition` for `DigitalInventoryEdition`, while
  the real index is `uq_digital_inventory_edition_id`. A comment
  flags the divergence; no caller currently uses `.to_string()` on
  this variant for DDL, but a future contributor copying the pattern
  could silently create a phantom `pk_…` index.
...
- The `PrimaryKey` enum's mixed semantics (composite PK + single-column
  ...
  single-column case, `PrimaryKey` for the composite case. Out of
  scope here; flagged in the in-line comment.
