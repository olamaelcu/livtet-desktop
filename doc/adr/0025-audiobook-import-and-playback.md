...
4. Play audiobooks in the dedicated `/reader/audio/[editionId]` window, persisting `timestamp` position to `reading_progress`. Duration and chapters reach the UI through the typed `reader_publication` descriptor; `EditionDetail` stays BigInt-safe because specta forbids `serde_json::Value`.
